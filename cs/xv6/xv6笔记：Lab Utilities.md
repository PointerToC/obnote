...
```
# 系统调用原理-以sys_sleep为例
在这个lab的第一个任务中，要求实现一个用户态的命令行函数`sleep()`,实现的方式很简单，按照步骤描述一步步来就好了。
```
// xv6/user/sleep.c
...
{
  if (argc != 2) {
    fprintf(2, "Uasge: sleep <time>\n");
    exit(1);
  }

  int sleep_time = atoi(argv[1]);
  if (0 == sleep_time) {
    fprintf(2, "Uasge: sleep <time> should not be 0\n");
    exit(1);
  }
...
```
我好奇的地方：
**sleep(sleep_time)是怎么最终调用到sys_sleep这个系统调用的？**
根据lab hints:
```
See kernel/sysproc.c for the xv6 kernel code that implements the sleep system call (look for sys_sleep), user/user.h for the C definition of sleep callable from a user program, and user/usys.S for the assembler code that jumps from user code into the kernel for sleep.
```
查看对应的源码文件:user/uys.S和kernel/syscall.c
```
// user/uys.S
...
```
解读：
1.sleep()是对**系统调用**的一个封装，它是一个用户态的程序。它来自于usys.S文件中通过.global将sleep符号导出给链接器，使得用户态的代码可以看得见这个"符号"。
2.按照**ABI调用约定**，a7寄存器存放系统调用号,sleep函数其实只做了这几件事：a)将系统调用号SYS_sleep放入寄存器a7。b)执行ecall。需要注意的是sleep函数的参数存放在a0寄存器。
3.执行ecall之后调用usertarp,在usertrap中执行syscall。**static uint64 (\*syscalls[])(void)** 就是个系统调用表。
完整的调用链是这样的：
```
用户态sleep()调用
   ↓
